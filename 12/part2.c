...
void trim_chain(chain_t *chain) {
    while (chain->first->current == 0) {
        pot_t *d = chain->first;
        chain->first = chain->first->next;
        chain->first->prev = NULL;
        free(d);
    }
    while (chain->last->current == 0) {
        pot_t *d = chain->last;
...
long sum_pot_ids(chain_t *chain) {
    long sum = 0;
    for (pot_t *iter = chain->first; iter; iter = iter->next)
...
int main() {
    ...
    for (long i = 0; i < 400000L; i++) {
        ...
        if ((i+1)%1000 == 0) printf("\r%05li  ", i+1);
        //print_chain(chain, i+1);
    }
    printf("there are %i plants\n", sum_plants(chain));
    printf("the sum of plant containing pot ids is %li\n", sum_pot_ids(chain));
